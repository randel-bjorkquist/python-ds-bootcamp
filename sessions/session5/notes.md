# Session 5: Conditionals, Loops, and Input Handling - Control Flow (if/elif/else, logic, loops)

## Topics Covered
- **if / elif / else** statements  
  - Chaining vs. separate comparisons  
  - Readability and performance trade-offs  
- **Comparison and logical operators** (`<`, `>`, `<=`, `>=`, `==`, `!=`, `and`, `or`, `not`)
- **Input validation**  
  - Manual checks with `if`  
  - Reusable `ask()` function with parser, choices, and validation rules
  - Rules-based approach using lambdas for cleaner branching
- **for loops**  
  - Skipping items with `continue`
  - Using `enumerate()` for indexed iteration
  - CSV-style parsing and formatted printing
- **while loops**  
  - Boolean flag vs. `while True` + `break`
  - Number guessing game with replay option

## Key Takeaways
- Multiple ways to express the same logic—choose clarity over cleverness.
- Separate comparisons can sometimes be more readable than chained ones.
- Reusable input helper functions (`ask()`) reduce repetitive validation code.
- Keep game state logic clear—regenerate values only when necessary.
- Loop style choice (`while True` vs. boolean) often comes down to preference and clarity.

## Exercises Completed
1. Age classification (`if`/`elif`/`else`)
2. Body temperature classification
3. Yes/No input with validation
4. Name list cleanup and title-casing
5. Enumerated shopping list
6. CSV row parsing with column headers
7. Number guessing game with replay
...
It’s perfect for cases when you don’t know ahead of time how many iterations you’ll need — such as waiting for valid input, retrying a network call, or processing until data is exhausted.
...
Key points:
* Always make sure your loop condition will eventually become False, or you’ll create an infinite loop.
* break will exit the loop early.
* continue will skip the rest of the loop body and start the next iteration.
